...
export class SignUpComponent extends BaseComponent  {
  ...
  signUpForm = new FormGroup({
    ...
    password1: new FormControl<string>('', { validators: [Validators.required, Validators.minLength(8)], nonNullable: true }),
    password2: new FormControl<string>('', { validators: [Validators.required, Validators.minLength(8)], nonNullable: true })
  ...
  signUp() {
    ...
    apiCall$.pipe(
      ...
      next: () => {
        this.messageService.success('You were successfully signed up', { nzDuration: 5000 });
        this.router.navigateByUrl('/auth/login');
      },
      error: (error) => {
        if (error.status === 401) {
          this.messageService.success('You were successfully signed up. Check your email for a verification link.', { nzDuration: 5000 });
          this.router.navigateByUrl('/auth/login');
          return;
        }
